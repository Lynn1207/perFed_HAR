  ...
  with tf.Graph().as_default():
    ...
    with tf.train.MonitoredTrainingSession(
      ...
      while outer_i < outer_iter:
        ...
        #assign_model(W_avg)
        mon_sess.run(tf.get_variable('conv1/weights:0',shape=[ 32, 1, 64],dtype=tf.float32,tf.truncated_normal_initializer(stddev=0.04, dtype=tf.float32)).assign(tf.reshape(W_avg[0:2048],[32, 1, 64])))
