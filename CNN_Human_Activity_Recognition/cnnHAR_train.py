  ...
  with tf.Graph().as_default():
    ...
    with tf.train.MonitoredTrainingSession(
      ...
      while outer_i < outer_iter:
        ...
        #assign_model(W_avg)
        mon_sess.run(tf.get_variable('conv1/weights:0',shape=[ 32, 1, 64],dtype=tf.float32,initializer=tf.truncated_normal_initializer(stddev=0.04, dtype=tf.float32)).assign(tf.reshape(W_avg[0:2048],[32, 1, 64])))
        ...
        mon_sess.run(tf.assign(tf.get_default_graph().get_tensor_by_name('local2/weights:0'), tf.reshape(W_avg[8288:73824],[64, 1024])))
        mon_sess.run(tf.assign(tf.get_default_graph().get_tensor_by_name('local2/bias:0'), W_avg[73824:74848]))
        mon_sess.run(tf.assign(tf.get_default_graph().get_tensor_by_name('local3/weights:0'), tf.reshape(W_avg[74848:599136],[1024, 512])))
        mon_sess.run(tf.assign(tf.get_default_graph().get_tensor_by_name('local3/bias:0'), W_avg[599136:599648]))
        mon_sess.run(tf.assign(tf.get_default_graph().get_tensor_by_name('local4/weights:0'), tf.reshape(W_avg[599648:615008],[512, 30])))
