    ...
    with tf.train.MonitoredTrainingSession(
      ...
      while outer_i < outer_iter:
        ...
        with tf.variable_scope('conv1') as scope:
          mon_sess.run(tf.get_variable('weights1:0',shape=[ 32, 1, 64],dtype=tf.float32,tf.truncated_normal_initializer(stddev=0.04, dtype=tf.float32)).assign(tf.reshape(W_avg[0:2048],[32, 1, 64])))
