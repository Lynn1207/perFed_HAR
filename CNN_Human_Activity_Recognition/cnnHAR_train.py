  ...
  with tf.Graph().as_default():
    ...
    while outer_i < outer_iter:
      ...
      with tf.train.MonitoredTrainingSession(
        ...
        #assign_model(W_avg)
        mon_sess.run(tf.assign(tf.get_default_graph().get_tensor_by_name('conv1/weights'), tf.reshape(W_avg[0:2048],[32, 1, 64])))
        mon_sess.run(tf.assign(tf.get_default_graph().get_tensor_by_name('conv1/bias'), W_avg[2048:2112]))
        mon_sess.run(tf.assign(tf.get_default_graph().get_tensor_by_name('conv2/weights'), tf.reshape(W_avg[2112:8256],[3, 64, 32])))
        mon_sess.run(tf.assign(tf.get_default_graph().get_tensor_by_name('conv2/bias'), W_avg[8256:8288]))
        mon_sess.run(tf.assign(tf.get_default_graph().get_tensor_by_name('local2/weights'), tf.reshape(W_avg[8288:73824],[64, 1024])))
        mon_sess.run(tf.assign(tf.get_default_graph().get_tensor_by_name('local2/bias'), W_avg[73824:74848]))
        mon_sess.run(tf.assign(tf.get_default_graph().get_tensor_by_name('local3/weights'), tf.reshape(W_avg[74848:599136],[1024, 512])))
        mon_sess.run(tf.assign(tf.get_default_graph().get_tensor_by_name('local3/bias'), W_avg[599136:599648]))
        mon_sess.run(tf.assign(tf.get_default_graph().get_tensor_by_name('local4/weights'), tf.reshape(W_avg[599648:615008],[512, 30])))
        mon_sess.run(tf.assign(tf.get_default_graph().get_tensor_by_name('local4/bias'), W_avg[615008:615038]))
        mon_sess.run(tf.assign(tf.get_default_graph().get_tensor_by_name('softmax_linear/weights'), tf.reshape(W_avg[615038:615218],[30, 6])))
        mon_sess.run(tf.assign(tf.get_default_graph().get_tensor_by_name('softmax_linear/bias'), W_avg[615218:615224]))
