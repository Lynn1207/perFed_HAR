...
def inference(signals):
    ...
    with tf.variable_scope('local3') as scope:
        ...
        weights = _variable_with_weight_decay('weights4', shape=[1024, 512],
                                              stddev=0.04, wd=0.004)#0.004,index)
    ...
    with tf.variable_scope('local4') as scope:
        weights = _variable_with_weight_decay('weights5', shape=[512, 30], stddev=0.04, wd=0.004)
    ...
    with tf.variable_scope('softmax_linear') as scope:
          weights = _variable_with_weight_decay('weights6', [30, NUM_CLASSES],stddev=0.04, wd=0.004)
